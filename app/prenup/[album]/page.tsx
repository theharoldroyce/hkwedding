...
const ALBUMS: Record<string, { title: string; subtitle: string }> = {
  v1: { title: "Volume One", subtitle: "Golden hour in the open fields" },
  v2: { title: "Volume Two", subtitle: "Quiet moments, just the two of us" },
};
...
export default async function PrenupAlbumPage({ params }: { params: Promise<{ album: string }> }) {
  ...
  const photos: PrenupPhoto[] = (data ?? []).map((row) => ({
